...
export default function SakinahPage() {
  ...
  const { t, ln, locale, prefs, setPrefs, current, setCurrent, history, ready } =
    useApp();
  ...
  const passages = getMood(mood).passages;
  ...
  function begin(selected: MoodId) {
    setCurrent({
      mood: selected,
      startedAt: Date.now(),
      index: 0,
  ...
  return (
    <AppShell>
      ...
      <main className="mx-auto max-w-content px-8 pb-20 pt-8">
        {/* Davom ettirish banneri — tugallanmagan sessiya bo'lsa */}
        {ready && current && !current.done && (
          <div className="mb-6 flex flex-wrap items-center gap-4 rounded-2xl border border-brand/30 bg-brand-soft px-6 py-4">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 truncate text-xs text-ink-secondary">
                {ln(getMood(current.mood).label)} ·{" "}
                {getMood(current.mood)
                  .passages.map((p) => passageRef(p))
                  .join(" · ")}
              </p>
            </div>
        ...
        {/* Sarlavha bloki — Figmadagi hero + سَكِينَة kalligrafiyasi */}
        <section className="relative overflow-hidden rounded-2xl border border-brand/20 bg-brand-soft/60 px-10 py-9">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button
              ...
              className="h-11 rounded-full border border-line-bold bg-surface px-6 text-sm font-semibold text-ink transition hover:border-ink-muted"
            >
              {t("entry.how")}
            </button>
          ...
          {howOpen && (
            <p className="mt-4 max-w-2xl rounded-xl bg-surface/70 px-5 py-4 text-sm leading-relaxed text-ink-secondary">
              {t("entry.subtitle")}
            </p>
          )}
        </section>
        ...
        <section className="mt-8">
            ...
            {MOODS.map((m) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  className={[
                    "flex h-[88px] items-center justify-between rounded-2xl border px-6 text-left transition",
                    active
                ...
                >
                  ...
                  <span
                    className="arabic font-arabic text-xl text-ink-muted"
                    aria-hidden="true"
                  >
                    {m.arabic}
                  </span>
          ...
          {/* Uchta sozlama — Figmadagi 2/3/4-savollar shu yerga yig'ilgan */}
          <div className="mt-6 flex flex-wrap gap-3">
            <Select
              label={t("compose.title")}
              value={prefs.intention}
              onChange={(v) => setPrefs({ intention: v })}
              options={INTENTIONS.map((i) => ({
                value: i.id,
                label: ln(i.label),
              }))}
            />
            <Select
              ...
              options={DURATIONS.map((d) => ({
                value: d,
                label: `${d} ${t("common.minutes")}`,
              }))}
            />
            <Select
              label={t("read.audio")}
              value={prefs.format}
              onChange={(v) => setPrefs({ format: v })}
            ...
            {RECITERS.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  {active && <Icon name="check" size={18} className="text-brand" />}
                </button>
              );
            })}
          </div>
